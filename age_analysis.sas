...
sum(grscnt_sum) as grssum, avg(grs_percent) as grsaverage,
sum(netcnt_sum) as netsum, avg(net_percent) as netaverage,
sum(tap_sum) as tapsum, sum(cost) as costsum,
avg(c_tap) as ctapaverage from ck1.age_a1
;
quit;
